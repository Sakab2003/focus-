...
<div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-light navbar-light">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <a href="/parent/eleve/inscription" class="nav-item nav-link"><i class="fa fa-th me-2"></i>Inscriptions</a>
    ...
    <h3>Traitement de l'exercice : {{ $exercice->titre }}</h3>

    {{-- EXERCICE EN LECTURE SEULE --}}
    @if($exercice->contenu)
        <h5 class="mt-3">Énoncé</h5>
        <div class="p-3 bg-light border rounded mb-4" style="pointer-events:none; user-select:none;">
            {!! $exercice->contenu !!}
        </div>
    @endif

    @if($exercice->fichier || $exercice->fichiers)
    <h5>Fichiers fournis</h5>

        @if($exercice->fichier)
            <a href="{{ asset('storage/'.$exercice->fichier) }}" target="_blank" class="btn btn-primary mb-2">Voir/Télécharger</a>
        @endif

        @if($exercice->fichiers)
            @foreach($exercice->fichiers as $file)
                <a href="{{ asset('storage/'.$file) }}" class="btn btn-secondary d-block mb-2" target="_blank">{{ basename($file) }}</a>
            @endforeach
        @endif
    @endif

    {{-- FORMULAIRE DE REPONSE --}}
    <form action="{{ route('parent.exercice.store',$exercice->id) }}"
      method="POST"
      enctype="multipart/form-data">
    @csrf

    <label>Votre réponse :</label>
    <textarea name="reponse_texte" id="editor" class="form-control"></textarea>

    <label class="mt-3">Uploader un fichier (optionnel)</label>
    <input type="file" name="reponse_fichier" class="form-control">

    <button class="btn btn-success mt-3">Envoyer ma réponse</button>
</form>
