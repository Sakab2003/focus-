...
<div class="nav-item dropdown">                    <a href="/parent/eleve/liste" class="nav-item nav-link"><i class="fa fa-th me-2"></i>Devoirs</a>
        ...
        <div class="content">
            ...
            <!-- Navbar End -->
            <h2>Traitement du devoir</h2>

<p>Élève : {{ $eleve->nom }} {{ $eleve->prenom }}</p>
<p>Devoir : {{ $devoir->cours->titre ?? 'Titre indisponible' }}</p>
@if(session('success'))
...
<!-- Formulaire ou actions de traitement ici -->
<form action="{{ route('parent.reponse_devoir') }}" method="POST" enctype="multipart/form-data">
    @csrf
    <input type="hidden" name="devoir_id" value="{{ $devoir->id }}">
    <input type="hidden" name="eleve_id" value="{{ $eleve->id }}">

    <div class="mb-3">
        <label for="reponse" class="form-label fw-bold">Votre réponse (texte) :</label>
        <textarea name="reponse" id="reponse" class="form-control" rows="10"></textarea>
    </div>

    <div class="mb-3">
        <label for="fichier" class="form-label fw-bold">Ajouter un fichier (PDF, doc, image...)</label>
        <input type="file" name="fichier" id="fichier" class="form-control" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.txt">
    </div>
