...
<div class="container-xxl position-relative bg-white d-flex p-0">
    ...
    <div class="content">
        ...
        <div class="container mt-4">
            <h1 class="mb-4">Visualisation du Cours</h1>

            {{-- Titre --}}
            @if(!empty($cours->titre))
                <div class="mb-3">
                    <div class="field-label">Titre :</div>
                    <div class="field-content">{{ $cours->titre }}</div>
                </div>
            @endif
...
@if($cours->classe)
    <div class="mb-3">
        ...
        <div class="field-content">
            {{ $cours->classe->name ?? 'N/A' }}
        </div>
    </div>
@else
    <div class="mb-3">
        <div class="field-label">Classe :</div>
        <div class="field-content text-muted">Classe non trouvée</div>
    </div>
@endif
        ...
        {{ $cours->matiere->name ?? 'N/A' }}
            ...
            @if(!empty($cours->contenu))
                <div class="mb-3">
                    ...
                    <div class="field-content">{!! $cours->contenu !!}</div>
                </div>
            @endif
                ...
                <div class="mb-3">
                    <ul class="file-list">
                        <li>
                            ...
                            <a href="{{ asset('storage/'.$cours->fichier) }}" target="_blank">{{ basename($cours->fichier) }}</a>
                        </li>
                    </ul>
                </div>
            @endif
            ...
            {{-- Autres fichiers --}}
            @if(!empty($cours->fichiers))
                <div class="mb-3">
                    <ul class="file-list">
                        @foreach($cours->fichiers as $file)
                            @php
