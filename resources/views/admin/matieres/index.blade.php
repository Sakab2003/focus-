...
<body>
    ...
    <a href="{{ route('admin.matieres.create') }}" class="btn btn-success mb-3">
        + Nouvelle Matiere
    </a>
        ...
        <tbody>
            @foreach($matieres as $mat)
                <tr>
                    ...
                    <td>{{ $mat->name }}</td>
                    <td>{{ $mat->code }}</td>
                    <td>{{ $mat->school_year }}</td>
                    </td>
    <td>
        <!-- Modifier -->
        <a href="{{ route('admin.matieres.edit', $mat->id) }}" class="btn btn-info">Modifier</a>

        <!-- Supprimer via modal Bootstrap -->
        <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal{{ $mat->id }}">
            Supprimer
        </button>

        <!-- Modal suppression -->
        <div class="modal fade" id="deleteModal{{ $mat->id }}" tabindex="-1" aria-labelledby="deleteModalLabel{{ $mat->id }}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title text-danger" id="deleteModalLabel{{ $mat->id }}">
                            Confirmation de suppression
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <p>Voulez-vous vraiment supprimer la matiere <strong>{{ $mat->titre }}</strong> ?</p>
                        <p class="text-muted mb-0">Cette action est irréversible.</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
                        <form action="{{ route('admin.matieres.destroy', $mat->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
        ...
        <!-- Voir -->
        <a href="{{ route('admin.matieres.show', $mat->id) }}" class="btn btn-dark">Voir</a>
    </td>
                </tr>
            @endforeach
        </tbody>
    </table>
</div><button onclick="window.history.back()" class="btn btn-primary">
    Retour
</button>
