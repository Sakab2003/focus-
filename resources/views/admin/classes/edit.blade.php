...
<body>
    ...
    <form action="{{ route('admin.classes.update', $classe->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="name" value="{{ old('name', $classe->name) }}"
        ...
        <div class="mb-3">
    <label class="form-label">Nombre maximum d’enseignants</label>
    <input type="number"
           name="max_enseignants"
           class="form-control @error('max_enseignants') is-invalid @enderror"
           min="1"
           max="100"
           placeholder="Ex : 5 (maximum 100)"
           required>

    @error('max_enseignants')
        <div class="invalid-feedback">{{ $message }}</div>
    @enderror
</div>
